...
cv::Rect get_rect(int image_width, int image_height, float bbox[4]) {
    int l, r, t, b;
    float r_w = Yolo::INPUT_W / (image_width * 1.0);
    float r_h = Yolo::INPUT_H / (image_height * 1.0);
    if (r_h > r_w) {
        l = bbox[0] - bbox[2]/2.f;
        r = bbox[0] + bbox[2]/2.f;
        t = bbox[1] - bbox[3]/2.f - (Yolo::INPUT_H - r_w * image_height) / 2;
        b = bbox[1] + bbox[3]/2.f - (Yolo::INPUT_H - r_w * image_height) / 2;
        l = l / r_w;
        r = r / r_w;
        t = t / r_w;
        b = b / r_w;
    } else {
        l = bbox[0] - bbox[2]/2.f - (Yolo::INPUT_W - r_h * image_width) / 2;
        r = bbox[0] + bbox[2]/2.f - (Yolo::INPUT_W - r_h * image_width) / 2;
        t = bbox[1] - bbox[3]/2.f;
        b = bbox[1] + bbox[3]/2.f;
        l = l / r_h;
        r = r / r_h;
